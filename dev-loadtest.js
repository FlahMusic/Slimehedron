...
// ============================================================================================
//  CSS SANITY. A single mismatched quote inside a url(...) once silently swallowed ~20 rules:
//  the browser threw nothing, the JS ran fine, the page looked *almost* right, and the only
//  symptom was that some phone sizing quietly stopped applying. JS-only checks cannot see that,
//  so the stylesheet gets parsed here too. Cheap, and it catches a whole class of silent bugs.
// ============================================================================================
(function(){
  const blocks=[...html.matchAll(/<style[^>]*>([\s\S]*?)<\/style>/g)].map(x=>x[1]);
  if(!blocks.length){console.log('CSS: no <style> block found');return;}
  const css=blocks.join('\n');const problems=[];
  // walk the stylesheet: skip /* comments */ (they are full of apostrophes), then check that every
  // string opened on a line is closed on that line, and that braces balance.
  let q=null,line=1,inC=false,d=0,under=false;
  for(let i=0;i<css.length;i++){
    const c=css[i],n=css[i+1];
    if(c==='\n'){line++;
      if(q){problems.push('unterminated '+q+' string on CSS line '+(line-1)+': '+css.split('\n')[line-2].trim().slice(0,90));q=null;}
      continue;}
    if(inC){ if(c==='*'&&n==='/'){inC=false;i++;} continue; }
    if(q){ if(c==='\\'){i++;} else if(c===q)q=null; continue; }
    if(c==='/'&&n==='*'){inC=true;i++;continue;}
    if(c==='"'||c==="'"){q=c;continue;}
    if(c==='{')d++; else if(c==='}'){d--;if(d<0)under=true;}
  }
  if(d!==0)problems.push('unbalanced braces: '+d+' block(s) left open at end of stylesheet');
  if(under)problems.push('a closing brace appeared with no matching open brace');
  if(problems.length){ok=false;problems.forEach(p=>console.log('CSS PROBLEM: '+p));}
  else console.log('CSS OK — quotes balanced, braces balanced ('+css.split('\n').length+' lines)');
})();
